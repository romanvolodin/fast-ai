...
Для установки фронтенда необходимо:

- Скачать [архив с скомпилированным фронтендом](https://dvmn.org/filer/canonical/1750917110/1035/)
- Файлы архива добавить в папку `static` рядом с `main.py`.

Коммитить файлы фронтенда в репозиторий не надо, им занимается отдельная команда.

### Настройка backendBaseUrl

В папке `static/` необходимо создать файл `frontend-settings.json` с настройками фронтенда. Самой важной является `backendBaseUrl` — адрес, на котором работает бэкенд. Пример:
